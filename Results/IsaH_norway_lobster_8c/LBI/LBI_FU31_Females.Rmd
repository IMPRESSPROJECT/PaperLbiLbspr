---
title: "LBI Análisis de Sensibilidad - FU 31 - FEMALES - M/K=1.9"
...
Ahora para la cigala FU 31 Females. Los datos del ciclo vital recogidos de la literatura son:

$L_{mat}^{LIT}$ = 28.0 mm. FU 25 (Fariña, 1996)    

$L_{inf}^{LIT}$ = 96.0 mm. (Distribución de tallas, 2003).     

${M/k}^{LIT}$ = 1.9 (Princeet al., 2015).
...
wal <- read.csv("31_Females_w.csv", stringsAsFactors = FALSE)
head(wal)
freq <- read.csv("31_Females.csv",  stringsAsFactors = FALSE)
head(freq)
```
...
lb_tableSH(freq, 1, "mm", linf = 96.0 , lmat = 28.0 , mk_ratio = 1.90 ,wal)
```
...
```{r results="asis", warning=FALSE}
lb_tableSH(freq, 1, "mm", linf=96.0, lmat=28.0, mk_ratio=0.75*1.9,wal)
```

2. Segundo sobrestimando (${M/k}^{LIT}$ multiplicado por 1.25):

```{r results="asis", warning=FALSE}
lb_tableSH(freq, 1, "mm", linf=96.0, lmat=28.0, mk_ratio=1.25*1.9,wal)
```

3. Tercero usando el valor por defecto de 1.5.

```{r results="asis", warning=FALSE}
lb_tableSH(freq, 1, "m", linf=96.0, lmat=28.0, mk_ratio=1.5,wal)
```
...
```{r results="asis", warning=FALSE}
lb_tableSH(freq, 1, "mm", linf=0.75*96.0, lmat=28.0, mk_ratio=1.9,wal)
```

2. Segundo sobrestimando ($L_{inf}^{LIT}$ multiplicado por 1.25):

```{r results="asis", warning=FALSE}
lb_tableSH(freq, 1, "mm", linf=1.25*96.0, lmat=28.0, mk_ratio=1.9,wal)
```
